...
//		lstCanal_out_dyke <- river overlapping 	
		...
		loop v_dyke over:dyke where (each.dyke_region >1){ // các đoạn sông ngoài đê
			ask river overlapping (v_dyke-10#m){ // tìm các đoạn sông nằm ngoài đô
				out_dyke<-true;
				color <-#red;
				lstCell <- (active_cell overlapping (self+50#m)); // tìm các cell nằm trong bán kính 50m từ kênh
...
grid land_cell file: cell_file control: reflex neighbors: 8 {
	...
	int suitability <-0; // những điềm gần sông rạch được gán hệ số này = 1 để lan truyền thuận lợi 
	init {
	}

...
global control: reflex {
	...
	float w_lancan <- 0.8;
	file landsuitability_2_file <- csv_file('../includes/dataset/landsuitability_2.csv', false);
	...
	action doc_matran_Loinhuan {
		...
		write "Ma tran Thich Nghi" + bebefit_matrix;
	}
	action tinh_kappa {
...
grid land_cell file: cell_file control: reflex neighbors: 8 {
	...
	list<list> landuse_eval {
		...
		candluc << check_suitability(madvdd, 5);
		...
		candtsl << check_suitability(madvdd, 34);
		...
		candluk << check_suitability(madvdd, 6);
		...
		candcln << check_suitability(madvdd, 14);
		...
		candbhk << check_suitability(madvdd, 12);

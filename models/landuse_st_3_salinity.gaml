...
//	file landunit_file <- shape_file('../includes/dvdd_hientai_region.shp');
	...
	map<int,rgb>salinity_color<-[0::#white,1::rgb(250,210,210),2::rgb(240,160,160),3::rgb(230,80,80),4::rgb(230,30,30),5::rgb(150,20,20)];
...
//		action tinh toan chuyen doi salinity and salt time
		ask shuffle(active_cell where (each.dyke_region=2)){
		//do check_LU_density;
//			do cal_capability_index;
//action change_landuse {
// Nên chia vùng chuyển đôi mặn ra: Vùng từ mức 3->4; Vùng từ mức 3->5

			int salinity_tmp<-salinity_level;
//			if (flip(15)) {
				list<float> vals <- salinity_markov_map[dyke_region][salinity_level];
//				if sum(vals) = 0 {salinity_tmp <- landuse;}
//				else {
...
grid land_cell file: cell_file control: reflex neighbors: 8 {
	...
	rgb color_salinity<-salinity_color[salinity_level];
	rgb color_salinity10<-salinity_color[salinity_level_obs];
...
//		}
		...
		display salinity_2010 type:java2D{
			species land_cell aspect:salinity10 ;
		}
	}
}
